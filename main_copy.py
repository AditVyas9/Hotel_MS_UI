from PyQt6.QtGui import (QAction, QIcon, QFont, QPainter, QPainterPath,
						 QColor, QMovie, QRegion)
from PyQt6.QtWidgets import (
	QApplication, QWidget, QGridLayout, QRadioButton, QButtonGroup,
	QPushButton, QLabel, QStackedLayout, QMainWindow, QTableWidget,
	QTableWidgetItem, QHBoxLayout, QDialog,
	QLineEdit, QDialogButtonBox, QMessageBox, QVBoxLayout, QSpinBox, QGroupBox,
	QFormLayout, QHeaderView, QSizePolicy,
	QToolButton, QScrollArea, QDateEdit, QAbstractItemView,
	QGraphicsDropShadowEffect, QProgressDialog, QFrame, QMenuBar,
	QStyle, QTimeEdit, QGraphicsOpacityEffect, QGraphicsBlurEffect, QStatusBar
)
from PyQt6.QtCore import (QTimer, Qt, QDate, pyqtSignal,
						  QThread, QRectF, QPropertyAnimation, QObject,
						  QRunnable, QThreadPool,
						  QTime, QEasingCurve, QSize, QPoint, QEvent)
import time, pymysql, sys, json, traceback, re, ast, pathlib, socket, \
	ctypes.wintypes, threading, schedule
...
class RoundedDialog(QDialog):
	...
	def keyPressEvent(self, event):
		...
		if (event.modifiers() & Qt.KeyboardModifier.ControlModifier and
			...
			event.ignore()
			return
		if (event.modifiers() & Qt.KeyboardModifier.MetaModifier and
				event.key() == Qt.Key.Key_W):
			event.ignore()
			return
	...
	def show_about(parent=None):
		...
		close_btn.setGraphicsEffect(
			QGraphicsDropShadowEffect(blurRadius=10, xOffset=0, yOffset=2,
...
class HotelIDialog(RoundedDialog):
	...
	def verify_and_accept(self):
		...
		if not hotel_ID:
			MessageBoxManager.warning(self, "Missing Hotel ID",
									  "Please enter a valid Hotel ID.")
			return

		if not verify_hotel_id(hotel_ID):
			MessageBoxManager.warning(self, "Invalid Hotel ID",
									  "Hotel ID not found in the system.")
			return
...
def verify_hotel_id(hotel_ID: str) -> bool:
	...
	try:
		cursor = conn.cursor()
		cursor.execute("SELECT 1 FROM hotel_details WHERE hotel_id = %s",
					   (hotel_ID,))
		result = cursor.fetchone()
		return result is not None
	except Exception as e:
		MessageBoxManager.warning(None, "DB ERROR",
								  f"Failed to verify hotel ID: {e}")
		return False
	finally:
		conn.close()


def display_table_data(table: QTableWidget, data: list, columns: list,
...
class BookingViewer(RoundedDialog):
	...
	def load_bookings(self, hotel_id):
		...
		for b in bookings:
			...
			names = [room_type_map.get(i.strip(), f"Unknown({i})") for i in
					 ids]
	...
	def display_bookings(self, bookings):
		columns = [
			"booking_id", "name", "room_no", "room_type",
			"date_from", "date_to", "phone", "aadhar", "age"
		]
		display_table_data(self.table, bookings, columns,
						   no_data_message="No bookings available")

	def filter_bookings(self, text):
		text = text.lower()
		filtered = [
			row for row in self.bookings
			if text in str(row["booking_id"]).lower() or text in str(
				row["name"]).lower()
		]
...
class HomePage(QMainWindow):
	...
	def __init__(self):
		...
		title_label.setStyleSheet(
			"font-size: 28px; font-weight: bold; color: #2E86C1;")
		...
		self.filter_input_page2.setPlaceholderText(
			"Filter by Hotel Name or Place")
		self.filter_input_page2.setStyleSheet(self._line_edit_style())
		self.filter_input_page2.textChanged.connect(
			self.refresh_hotel_list_page2)
		...
		self.filter_input_page3.setPlaceholderText(
			"Filter by Hotel Name or Place")
		self.filter_input_page3.setStyleSheet(self._line_edit_style())
		self.filter_input_page3.textChanged.connect(
			self.refresh_hotel_list_page3)
		...
		for menu in (self.toggle_menu_item, self.action_menu_item,
					 self.action2_menu_item, self.about_menu_item):
			menu.setWindowFlags(
				Qt.WindowType.Popup |
				Qt.WindowType.FramelessWindowHint |
		...
		delete_booking = QAction("Delete Booking", self)
		delete_booking.setShortcut("Ctrl+Delete")
		update_booking = QAction("Update Booking", self)
		update_booking.setShortcut("Ctrl+E")
		book_hotel = QAction("Book Hotel", self)
		book_hotel.setShortcut("Alt+H")
		self.action2_menu_item.addActions(
			(action_check_booking, delete_booking, update_booking, book_hotel))
	...
	def apply_rounded_corners(self):
		radius = 20
		path = QPainterPath()
		path.addRoundedRect(QRectF(0, 0, self.width(), self.height()), radius,
							radius)
	...
	def home_action(self):
		loader = LoadingOverlay(self, gif_path="Icons/spinner.gif",
								text="Loading...")
		loader.show_overlay()
	...
	def _update_network_status(self, connected: bool):
		...
		self.network_label.setText(
			f"\U0001F310 Internet: {self.network_status}")
	...
	def populate_table(self, table: QTableWidget, data: list, columns: list,
					   action_name: str = None, log_msg: str = None,
					   no_data_message="No data available"):
		...
		if action_name:
			self.booking_manager.log_action1(action_name,
											 message=log_msg or f"Fetched {len(data)} rows for table")
	...
	def refresh_all_tables(self, force=True):
		...
		QTimer.singleShot(0, lambda: self.refresh_hotel_table(force=force,
															  background=True,
															  table=0))
		QTimer.singleShot(0, lambda: self.refresh_hotel_table(force=force,
															  background=True,
	...
	def open_hotel1_dialog(self):
		...
		if not login_dialog.exec():
			MessageBoxManager.warning(self, "Login Failed",
	...
	def cell_clicked_book(self):
		...
		class BookingLoadWorker(QObject):
			...
			def run(self):
				data = self.manager.get_hotel_data(self.hotel_id) if hasattr(
					self.manager, "get_hotel_data") else None
	...
	def refresh_hotel_table(self, table=0, force=False, background=True):
		...
		def fetch_fn():
			...
			action_name = (
				"select_management_hotels" if table == 0
				else "select_booking_hotels"
			)
			self.booking_manager.log_action1(
				action_name,
	...
	def change_page(self):
		if self.pages_layout.currentIndex() == 0: self.next_page = self.selected_option
		QApplication.processEvents()
		...
		self.page_worker = PageWorker(self.next_page, self.booking_manager,
									  self._table_cache)
	...
	def on_page_data_loaded(self, loader, booking_data, management_data,
							next_page):
		if booking_data:
		...
		self.next_page = "Booking Portal" if self.next_page == "Management Portal" else "Management Portal"
	...
	def _refresh_after_switch_async(self):
		if self.next_page == "Booking Portal":
			table = self.table1
			loader_fn = lambda: self.refresh_hotel_table(background=True,
														 table=1)
		else:
			table = self.table
			loader_fn = lambda: self.refresh_hotel_table(background=True,
...
class LoadingOverlay(QWidget):
	def __init__(self, parent=None, gif_path="Icons/spinner.gif",
				 text="Loading..."):
		super().__init__(parent)
		self.parent = parent
	...
	def eventFilter(self, obj, event):
		if obj == self.parent and event.type() in (event.Type.Move,
												   event.Type.Resize):
			self._align_with_parent()
		return super().eventFilter(obj, event)
...
class HotelForm(RoundedDialog):
	def __init__(self, parent=None, hotel_id=None):
		...
		title_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
		title_label.setStyleSheet(
			"font-size: 20px; font-weight: 600; color: #1F4E79;")
		...
		main_layout.insertWidget(0, close_x,
								 alignment=Qt.AlignmentFlag.AlignRight)
		...
		for w in [
			self.hotel_name_input, self.place_input, self.pin_input,
			self.contact_input, self.floor_input, self.rooms_input,
			self.room_no, self.username_input, self.password_input
		]:
		...
		scroll_hotel.setMinimumHeight(
			280)
		scroll_hotel.setMaximumHeight(400)
		scroll_hotel.setVerticalScrollBarPolicy(
			Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
		scroll_hotel.setHorizontalScrollBarPolicy(
			Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
		...
		self.ok_button = self.button_box.addButton("Save",
												   QDialogButtonBox.ButtonRole.AcceptRole)
		self.cancel_button = self.button_box.addButton("Cancel",
													   QDialogButtonBox.ButtonRole.RejectRole)

		if self.hotel_id:
			self.delete_button = self.button_box.addButton("Delete Hotel",
		...
		QTimer.singleShot(0, lambda: self.generate_room_fields(
			self.room_type_count.value()))
	...
	def get_all_other_rooms(self, exclude_index=None):
		...
		for i, field in enumerate(self.room_no_inputs):
			if exclude_index is not None and i == exclude_index:
				continue
			all_rooms.extend(
				[r.strip() for r in field.text().split(",") if r.strip()])
		return all_rooms
	...
	def on_dates_changed(self):
		...
		if booked_to < booked_from:
			MessageBoxManager.warning(self, "Date Error",
									  "Booked To date cannot be before Booked From date.")
	...
	def generate_room_fields(self, count):
		...
		old_totals = [w.value() for w in
					  getattr(self, "total_room_inputs", [])]
		...
		if current_rows > count:
			for i in range(current_rows - 1, count - 1, -1):
				...
				if hasattr(self, "assigned_labels") and i < len(
						self.assigned_labels):
		...
		for i in range(count):
			...
			self.dynamic_fields_layout.addWidget(QLabel(f"Room Type {i + 1}:"),
												 i, 0)
			...
			type_input.textChanged.connect(
				lambda text, lbl=assigned_label, i=i:
				lbl.setText(f"{text or f'Room Type {i + 1}'}:"))
			...
			total_input.valueChanged.connect(
				lambda _, rn_func=self.update_room_no_fields:
				QTimer.singleShot(0, rn_func))
	...
	def update_room_no_fields(self):
		...
		for i, room_field in enumerate(self.room_no_inputs):
			total = self.total_room_inputs[i].value() if i < len(
				self.total_room_inputs) else 0
	...
	def load_data(self):
		...
		try:
			...
			cu.execute("SELECT * FROM hotel_details WHERE hotel_id=%s",
					   (self.hotel_id,))
			hotel = cu.fetchone()
			cu.close()
			conn.close()
			...
			def populate():
				...
				for i, room in enumerate(rooms_load_data):
					...
					slice_rooms = all_rooms[
								  assigned_index:assigned_index + total] if assigned_index < len(
						all_rooms) else []
			...
			MessageBoxManager.error(self, "Error",
									f"Failed to load hotel: {e}")
			if hasattr(self, "booking_manager") and self.booking_manager:
				self.booking_manager.log_action(
					"hotel_load_error",
	...
	def room_no_generator(self):
		...
		try:
			...
			self.room_no.clear()
			return
		room_list = [f"{f}{r:02d}" for f in range(1, floors + 1) for r in
					 range(1, rooms + 1)]
	...
	def submit_data(self):
		try:
			...
			room_list = [r.strip() for r in room_text.split(",") if
						 r.strip().isdigit()]
			total_rooms = sum(sp.value() for sp in self.total_room_inputs)
			...
			check_out_time = self.checkout_time_input.time().toString(
				"HH:mm:ss")
			...
			if is_new_hotel:
				...
				if not username or not password:
					raise ValueError(
						"Username and password required for new hotel.")

				cu.execute("""INSERT INTO hotel_details
							  (hotel_name, place, pin_code, contact_hotel, username, password,
							   floor, room, room_no, check_in, check_out)
							  VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""",
						   (name, place, pin_code, contact, username, password,
							int(self.floor_input.text()),
							int(self.rooms_input.text()),
							room_list_db, check_in_time, check_out_time))
			...
			cu.execute("SELECT id FROM room_types WHERE hotel_id=%s",
					   (self.hotel_id,))
			...
			for i in range(len(self.room_type_inputs)):
				...
				if not room_type:
					MessageBoxManager.error(self, "Error",
											f"Room type name required at row {i + 1}.")
					return

				room_no_list = [r.strip() for r in room_no_text.split(",") if
								r.strip()]
				...
				if room_id:
					if not is_new_hotel:
						cu.execute("""UPDATE room_types
									  SET room_type=%s, total=%s, room_no=%s, price=%s
									  WHERE id=%s""",
								   (room_type, total, room_no_json, price,
									room_id))
					...
					cu.execute("""INSERT INTO room_types
								  (hotel_id, room_type, total, room_no, price)
								  VALUES (%s,%s,%s,%s,%s)""",
							   (self.hotel_id, room_type, total, room_no_json,
								price))
					new_id = cu.lastrowid
			...
			MessageBoxManager.info(self, "Success",
	...
	def start_room_log_worker(self):
		...
		self.thread = RoomLogWorker(self.booking_manager,
									self.hotel_id)
	...
	def on_logs_error(self, msg):
		self.progress.close()
		MessageBoxManager.error(self, "Error",
								f"Failed to create room logs:\n{msg}")

	def delete_hotel(self):
		confirm = QMessageBox.question(self, "Delete Hotel",
		...
		if confirm == QMessageBox.StandardButton.Yes:
			try:
				conn = create_connection()
				cu = conn.cursor()
				cu.execute("DELETE FROM room_types WHERE hotel_id=%s",
						   (self.hotel_id,))
				cu.execute("DELETE FROM hotel_details WHERE hotel_id=%s",
						   (self.hotel_id,))
				conn.commit()
				cu.close()
				conn.close()
				MessageBoxManager.info(self, "Deleted",
...
class PasswordLineEdit(QLineEdit):
	...
	def resizeEvent(self, event):
		...
		self.eye_button.move(self.rect().right() - 25,
...
class LoginDialog(RoundedDialog):
	def __init__(self, hotel_id, parent=None):
		...
		self.button_box = QDialogButtonBox()
		self.ok_btn = self.button_box.addButton("Login",
												QDialogButtonBox.ButtonRole.AcceptRole)
		self.cancel_btn = self.button_box.addButton("Cancel",
													QDialogButtonBox.ButtonRole.RejectRole)
		...
		inner_layout.addWidget(self.button_box,
							   alignment=Qt.AlignmentFlag.AlignCenter)
		outer_layout = QVBoxLayout(self)
		outer_layout.setContentsMargins(5, 5, 5, 5)
		outer_layout.addWidget(self.bg_frame)
	...
	def authenticate(self):
		...
		if not username and not password:
			MessageBoxManager.warning(self, "Validation Error",
		...
		if record:
			if username == record["username"] and password == record[
				"password"]:
		...
		MessageBoxManager.error(self, "Login Failed",
...
class BookingForm(RoundedDialog):
	proceed_to_payment = pyqtSignal(object, float, str, str, str, str, QDate,
									QDate, int)
	payment_success = pyqtSignal(object, float, str, str, str, str, QDate,
								 QDate)
	...
	def __init__(self, hotel_id):
		...
		self.container_layout.insertWidget(0, close_x,
										   alignment=Qt.AlignmentFlag.AlignRight)
		...
		for lbl in [
			self.hotel_label, self.place_label, self.pin_code_label,
			self.contact_label, self.check_in, self.check_out]:
			lbl.setStyleSheet("font-size:14px;")

		hotel_layout.addWidget(QLabel("Hotel Name:"), 0, 0,
							   alignment=Qt.AlignmentFlag.AlignRight)
		hotel_layout.addWidget(self.hotel_label, 0, 1,
							   alignment=Qt.AlignmentFlag.AlignLeft)
		hotel_layout.addWidget(QLabel("Check-in:"), 0, 2,
							   alignment=Qt.AlignmentFlag.AlignRight)
		hotel_layout.addWidget(self.check_in, 0, 3,
							   alignment=Qt.AlignmentFlag.AlignLeft)
		hotel_layout.addWidget(QLabel("Place:"), 1, 0,
							   alignment=Qt.AlignmentFlag.AlignRight)
		hotel_layout.addWidget(self.place_label, 1, 1,
							   alignment=Qt.AlignmentFlag.AlignLeft)
		hotel_layout.addWidget(QLabel("Check-out:"), 1, 2,
							   alignment=Qt.AlignmentFlag.AlignRight)
		hotel_layout.addWidget(self.check_out, 1, 3,
							   alignment=Qt.AlignmentFlag.AlignLeft)
		hotel_layout.addWidget(QLabel("Pin Code:"), 2, 0,
							   alignment=Qt.AlignmentFlag.AlignRight)
		hotel_layout.addWidget(self.pin_code_label, 2, 1,
							   alignment=Qt.AlignmentFlag.AlignLeft)
		hotel_layout.addWidget(QLabel("Contact:"), 3, 0,
							   alignment=Qt.AlignmentFlag.AlignRight)
		hotel_layout.addWidget(self.contact_label, 3, 1,
							   alignment=Qt.AlignmentFlag.AlignLeft)
		...
		for w in [
			self.name_input, self.contact_input, self.email_input,
			self.aadhar_no_input, self.booked_from_input,
			self.booked_to_input, self.age]:
			w.setStyleSheet(self._line_edit_style())
	...
	@staticmethod
	def check_card_details(card_no_obj, card_holder_name_obj, cvv_obj,
						   exp_obj):
		try:
			...
			for line in lines:
				...
				if (card_no.strip() == card_no_obj and
						holder_name.strip().upper().replace(" ",
															"_") == card_holder_name_obj and
						cvv.strip() == cvv_obj and
						exp.strip() == exp_obj):
					return True
		except Exception as e:
	...
	def load_hotel_info(self):
		try:
			...
			MessageBoxManager.error(self, "Error",
									f"Failed to fetch hotel info: {e}")
		self.booking_manager.log_action(
			"load_hotel_info",
			hotel_id=self.hotel_id,
	...
	def load_room_labels(self):
		...
		try:
			...
			MessageBoxManager.error(self, "Error",
									f"Failed to fetch room prices: {e}")
			return
		finally:
			cu.close()
			conn.close()
	...
	def load_rooms(self):
		...
		try:
			...
			if not room_types_data:
				MessageBoxManager.info(self, "Info",
			...
			for rt in room_types_data:
				...
				def make_on_spin_change(total_available_i, label_i,
										avail_label_i):
					def on_spin_change(value):
						...
						avail_label_i.setStyleSheet(
							f"color:{color_i}; font-size:13px;")
			...
			MessageBoxManager.error(self, "Error",
									f"Failed to process room availability: {e}")
			traceback.print_exc()
	...
	def validate(self):
		...
		if booked_to < booked_from:
			MessageBoxManager.warning(self, "Validation Error",
									  "Booked To cannot be before From date.")
			return
		...
		try:
			...
			MessageBoxManager.error(self, "Error",
									f"Failed to fetch available rooms: {e}")
			return
		...
		for widget in self.room_booking_widgets:
			...
			total_available = len(
				available_by_type.get(widget["room_type"], []))
		...
		self.summary(
			booked_rooms, total_amount, name, phone, email, aadhar,
			booked_from, booked_to, int(age)
		)

	def summary(self, booked_rooms, total_amount, name, phone, email, aadhar,
				booked_from, booked_to, age):
		...
		customer_layout.addWidget(
			QLabel(f"From: {booked_from.toString('yyyy-MM-dd')}"))
		customer_layout.addWidget(
			QLabel(f"To: {booked_to.toString('yyyy-MM-dd')}"))
		...
		for room in booked_rooms:
			rooms_layout.addWidget(
				QLabel(
					f"{room['room_type']}: {room['quantity']}x {days} days"))
		...
		main_layout.insertWidget(0, close_btn,
								 alignment=Qt.AlignmentFlag.AlignRight)

		outer_layout.addWidget(container)
		dialog.exec()

	def open_payment_dialog(self, booked_rooms, total_amount, name, phone,
							email, aadhar, booked_from, booked_to, age):
		dialog = RoundedDialog(self)
	...
	""")
		...
		def format_card_number(text):
			...
			new_text = " ".join(
				[digits_only[i:i + 4] for i in
				 range(0, len(digits_only), 4)])
	...
	""")
		...
		def process_payment():
			cardholder = name_input.text().strip().upper().replace(" ",
																   "_")
			...
			if len(cvv) != 3 or not cvv.isdigit():
				...
				return
			if not self.check_card_details(card_number, cardholder, cvv,
										   exp):
			...
			self.save_booking_to_db(booked_rooms, total_amount, name,
									phone,
									email, aadhar, booked_from, booked_to, age)
			self.payment_success.emit(booked_rooms, total_amount, name,
									  phone,
									  email, aadhar, booked_from,
									  booked_to)
	...
	def save_booking_to_db(self, booked_rooms, total_amount,
						   name, phone, email, aadhar, booked_from,
						   booked_to, age):
		try:
			rooms_with_id = []
			for widget in self.room_booking_widgets:
			...
			self.booking_id = self.booking_manager.add_or_update_booking(
				booking)
			self.booking_manager.log_action(
			...
			MessageBoxManager.error(self, "Error",
									f"Error saving booking:\n{str(e)}")
			traceback.print_exc()
			return False

	def show_success_dialog(self, booked_rooms, total_amount, name, phone,
							email, aadhar, booked_from, booked_to):
		...
		close_x.setStyleSheet("""
			...
			QPushButton:hover { background-color: #EAECEE; }
		""")
		close_x.clicked.connect(lambda: [
			dialog.reject(), self.close()])
	...
	def generate_invoice(self, booked_rooms, total_amount, name, phone,
						 email, aadhar, booked_from, booked_to):
		...
		MessageBoxManager.info(self, "Invoice Generated",
...
class BookingDataManagement:
	def __init__(self, host="localhost", user="root", password="Adit@2011"):
		self.host = host
		self.user = user
		self.password = password
		self.db_name = "hotel_management"
	...
	def cleanup_old_bookings(self):
		...
		try:
			...
			MessageBoxManager.success(None,
									  f"Deletion successful: [{datetime.now()}] ",
									  f"Deleted {affected} old bookings (>1 month).")

			self.log_action1("monthly_cleanup",
							 f"Deleted {affected} old bookings")

		except Exception as e:
			MessageBoxManager.warning(None,
									  f"Cleanup error: [{datetime.now()}] ", e)
	...
	def get_management_data(self):
		...
		try:
			...
			self.log_action1(action="management_data_loaded",
							 message=f"Loaded {len(data)} hotels")
	...
	def _setup_logging_table_for_hotel(hotel_id):
		...
		try:
			...
			MessageBoxManager.error(None, f"Hotel {hotel_id} Log Error",
									f"Error setting up log table: {e}")
		finally:
			if cur: cur.close()
			if conn: conn.close()
	...
	def _setup_logging_table():
		...
		try:
			...
			MessageBoxManager.error(None, f"Log Error",
									f"Error setting up log table: {e}")
		finally:
			if cur: cur.close()
			if conn: conn.close()
	...
	def log_action(self, action, hotel_id=None, booking_id=None, message=None):
		if not hotel_id:
			MessageBoxManager.error(None, "Error",
									"Hotel ID required for logging")
			return
		...
		class LogRunnable(QRunnable):
			def run(inner_self):
				...
				try:
					...
					MessageBoxManager.error(None,
											f"Logging Failed for Hotel {hotel_id}",
											str(e))
				finally:
					if cur:
						cur.close()
	...
	def add_or_update_booking(self, booking_data):
		...
		try:
			...
			booked_from_str = booking_data['date_from'].toString("yyyy-MM-dd") \
				if isinstance(booking_data['date_from'], QDate) else str(
				booking_data['date_from'])
			booked_to_str = booking_data['date_to'].toString("yyyy-MM-dd") \
				if isinstance(booking_data['date_to'], QDate) else str(
			...
			available_rooms = self.get_available_rooms(hotel_id,
													   booked_from_str,
													   booked_to_str)
			...
			for rt in booking_data['room_type']:
				...
				candidates = [r for r in available_rooms if
							  r['room_type_id'] == room_type_id]
				if len(candidates) < quantity:
					conn.rollback()
					self.log_action(
						"booking_failed", hotel_id, booking_id_def,
			...
			cur.execute("""
				...
				(booking_id, hotel_id, name, room_no, room_type_id, date_from, date_to, phone, aadhar, age)
			...
			conn.commit()
			self.log_action(
				"booking_added", hotel_id, booking_id_def,
	...
	def delete_booking(self, booking_id_def):
		...
		try:
			...
			if not booking:
				self.log_action("delete_failed", None, booking_id_def,
								"Booking not found")
				return False

			cur.execute("DELETE FROM hotel_bookings WHERE booking_id=%s",
						(booking_id_def,))
			conn.commit()

			self.log_action("booking_deleted", booking['hotel_id'],
							booking_id_def,
			...
			if conn: conn.rollback()
			MessageBoxManager.warning(None, "Delete Failed",
									  "Booking not found")
	...
	def get_available_rooms(self, hotel_id, date_from, date_to):
		...
		try:
			...
			for row in booked_data:
				rt_ids = [r.strip() for r in
						  str(row["room_type_id"]).split(",")]

				try:
					room_nos = json.loads(row["room_no"]) if str(
						row["room_no"]).startswith("[") else str(
						row["room_no"]).split(",")
				except Exception:
					room_nos = str(row["room_no"]).split(",")

				for rt in rt_ids:
					booked_map.setdefault(rt, set()).update(
						r.strip() for r in room_nos)
			...
			for row in room_data:
				...
				try:
					all_rooms = json.loads(row["room_no"]) if str(
						row["room_no"]).startswith("[") else str(
						row["room_no"]).split(",")
				except Exception:
			...
			if not available_rooms:
				MessageBoxManager.warning(None, "No Rooms",
										  f"No rooms available for {date_from} to {date_to}")
	...
	def get_room_nos_by_booking_id(booking_id: str):
		...
		try:
			...
			MessageBoxManager.warning(None, "Room No Error",
...
class MessageBoxManager(RoundedDialog):
	...
	COLORS = {
		...
		"warning": {
			"border": "#ef6c00", "text": "#e65100", "button": "#ef6c00"},
		"success": {
			"border": "#2e7d32", "text": "#1b5e20", "button": "#2e7d32"}
	}
...
class BookingInvoice:
	def __init__(self, hotel_name, hotel_address, room_no, customer_name,
				 customer_aadhar,
				 customer_phone, booked_from, booked_to, customer_email, rooms,
				 total_amount,
				 app_name, app_logo_path, booking_id):
	...
	def build_invoice(self):
		doc = SimpleDocTemplate(
			str(self.pdf_path), pagesize=A4, rightMargin=40, leftMargin=40,
		...
		if self.app_logo_path:
			try:
				logo = Image(self.app_logo_path, width=1 * inch,
							 height=1 * inch)
				logo.hAlign = "LEFT"
				story.append(logo)
			except Exception:
		...
		story.append(
			Paragraph(f"<b>{self.hotel_name}</b>", self.styles["Title"]))
		story.append(Paragraph(self.hotel_address, self.styles["Normal"]))
		story.append(Spacer(1, 10))
		story.append(Paragraph(f"<i>Booked via {self.app_name}</i>",
							   self.styles["Italic"]))
		story.append(Spacer(1, 20))

		booking_table_data = [
			[f"Booking No: {self.booking_id}"],
			[f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"],
		]
		booking_table = Table(booking_table_data, colWidths=[450])
		...
		story.append(
			Paragraph("<b>Customer Details:</b>", self.styles["Heading3"]))
		customer_info = [
			[f"Name: {self.customer_name}"],
			[f"Aadhar Card: {self.customer_aadhar}"],
			[f"Phone: {self.customer_phone}"],
			[f"Email: {self.customer_email}"],
			[
				f"Booking Period: {self.booked_from.toString("dd-MM-yyyy")} to {self.booked_to.toString("dd-MM-yyyy")}"],
		]
		customer_table = Table(customer_info, colWidths=[450])
		customer_table.setStyle(
			TableStyle([("BOTTOMPADDING", (0, 0), (-1, -1), 5)]))
		story.append(customer_table)
		story.append(Spacer(1, 15))

		story.append(
			Paragraph("<b>Hotel / Room Info:</b>", self.styles["Heading3"]))
		story.append(
			Paragraph(f"Room No.: {self.room_no}", self.styles["Normal"]))
		story.append(Spacer(1, 10))

		table_data = [["Room Type", "Qty", "Price/Room", "Total"]]
		for room in self.rooms:
			total_price = room["quantity"] * room["price"]
			table_data.append(
				[
					room["room_type"],
					str(room["quantity"]),
		...
		story.append(
			Paragraph("<i>Thank you for your booking!</i>",
					  self.styles["Italic"])
		)
