...
import time, pymysql, sys, json, traceback, re, ast, pathlib, socket, ctypes.wintypes, threading, schedule
...
class BookingInvoice:
    def __init__(
        ...
        cu = conn.cursor()
        cu.execute(
            "SELECT contact_hotel, check_in, check_out FROM hotel_details WHERE hotel_name=%s",
            (self.hotel_name,),
        )
